...
<AccordionGroup>
  ...
  <Accordion title="claim_lp_fees">
    ...
    **No-op при нулевых обеих сторонах:** если `claimable_a == 0 && claimable_b == 0`, инструкция возвращает успех без выполнения SPL-трансфера.

    **Эффект:** Выведенные суммы переводятся из резервных vault'ов пула в соответствующие ATA LP-холдера. Эмитирует `LpFeesClaimed { claimable_a, claimable_b, recipient, ... }` — dual-side событие claim.
...
<AccordionGroup>
  ...
  <Accordion title="nexus_claim_rewards">
    ...
    **Логика:**
    ...
    2. Перевод `claimable_a` token-A из `pool_vault_a` → `nexus_token_a_ata`, если `> 0`; то же для B.
