...
<Steps>
  ...
  <Step title="LP-комиссия → per-side аккумулятор (мгновенный вывод)">
...
<Note>
  **Единый LP-интерфейс для пулов StandardCurve.** Пользователи взаимодействуют через `add_liquidity`, `zap_liquidity` и `remove_liquidity` в парах OT/RWT и сторонних парах. LP получают пропорциональные доли и зарабатывают через per-side аккумулятор на `PoolState` с клеймом через `claim_lp_fees`.
...
<AccordionGroup>
  ...
  <Accordion title="nexus_claim_rewards">
    Вывод накопленных LP-вознаграждений из резервных vault'ов пула в Areal Treasury. PDA Nexus выступает как LP-холдер и выводит свою пропорциональную долю на **обеих сторонах токенов** пула. Только Authority.
    ...
    **Логика:**
    ...
    2. Перевод `claimable_a` token-A из `pool_vault_a` → `treasury_token_a_ata`, если `> 0`; то же для B.
    3. Обновление `nexus_lp_position.fees_claimed_per_share_a` и `_b` до текущих cumulative-значений.
    4. Эмитирование `NexusRewardsClaimed`.

    <Note>
      Используется тот же per-side аккумулятор, что и в `claim_lp_fees`. LP-позиции PDA Nexus зарабатывают комиссии как любой другой LP-холдер — вознаграждения идут в Areal Treasury, не возвращаются обратно в principal floor (инвариант principal-lock учитывает только `total_deposited_*`, не вознаграждения за свопы). Для RWT-парных пулов на практике сегодня ненулевая только RWT-сторона, но инструкция выполняет оба перевода безусловно — для forward-compat с не-RWT парами.
    </Note>
  </Accordion>
...
| LP-вознаграждения Nexus (токены A и/или B) | ATA Areal Treasury (A и B) | `nexus_claim_rewards` (per-side Q64.64 выплата) | Authority |
