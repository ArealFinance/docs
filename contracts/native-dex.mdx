...
<AccordionGroup>
  ...
  <Accordion title="claim_lp_fees">
    ...
    **Effect:** Claimed amounts transferred from each side's reserve vault to the LP holder's corresponding ATA. Emits `LpFeesClaimed`.
...
<AccordionGroup>
  ...
  <Accordion title="nexus_claim_rewards">
    Claim accumulated LP fee rewards from a pool's reserve vaults to the Areal Treasury. The Nexus PDA acts as an LP holder and claims its proportional share on **both token sides** of the pool. Authority only.
    ...
    **Logic:**
    ...
    2. Transfer `claimable_a` token-A from `pool_vault_a` → `treasury_token_a_ata` if `> 0`; same for B.
    3. Update `nexus_lp_position.fees_claimed_per_share_a` and `_b` to the current cumulative values.
    4. Emit `NexusRewardsClaimed`.

    <Note>
      Uses the same per-side accumulator as `claim_lp_fees`. The Nexus PDA's LP positions earn fees like any other LP holder — rewards go to the Areal Treasury, not redeposited into the principal floor (the principal-lock invariant accounts only for `total_deposited_*`, not for swap-fee rewards). For RWT-paired pools, only the RWT-side amount is non-zero in practice today, but the instruction issues both transfers unconditionally for forward-compat with non-RWT pairs.
    </Note>
  </Accordion>
...
| Nexus LP fee rewards (token A and/or B) | Areal Treasury ATAs (A and B) | `nexus_claim_rewards` (per-side Q64.64 payout) | Authority |
