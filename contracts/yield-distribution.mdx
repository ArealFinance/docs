...
The 15% RWT liquidity slice produced by `rwt_engine::claim_yield` is staged in a contract-owned `LiquidityHolding` PDA before being atomically drained into the DEX `LiquidityNexus` token account. Yield Distribution exposes one new instruction for this flow.
...
Surrounding flow (instructions live in other contracts, included for reference):

- `ownership_token::distribute_revenue` — sends OT-revenue USDC into the YD per-OT Accumulator
- `yield_distribution::convert_to_rwt` — converts Accumulator USDC into RWT, deposits RWT into the reward vault, updates vesting state (documented in the Funding section above)
- `rwt_engine::claim_yield` — RWT Engine claims its share via the standard YD `claim` flow; the 15% liquidity slice lands in the `LiquidityHolding` RWT ATA (see [RWT Engine contract](/contracts/rwt-engine))
- `ownership_token::claim_yd_for_treasury` — OT Treasury PDA claims its share of any distributor (cross-project yield)
...
<AccordionGroup>
  <Accordion title="withdraw_liquidity_holding">
    ...
    **Logic:**
    ...
    2. CPI to `native_dex::nexus_record_deposit(amount, token_kind=RWT)` (`LiquidityHolding` PDA signs).
    3. Increment `liquidity_holding.total_drained` by `amount`.
    4. Emit `LiquidityHoldingWithdrawn { amount, total_drained, timestamp }`.

    <Note>
      **Atomic guarantee.** Either both legs succeed (RWT transfer AND principal-floor update) or both revert. There is no intermediate state in which the on-chain Nexus balance and `total_deposited_rwt` disagree on this lane.
    </Note>
  </Accordion>
</AccordionGroup>
...
Singleton PDA that stages RWT received from the 15% liquidity slice of `rwt_engine::claim_yield` before it is atomically drained into the DEX `LiquidityNexus`. The PDA owns a single RWT ATA (`liquidity_holding_rwt_ata`) which is the staging account; the drain ix transfers RWT to the Nexus and updates the Nexus principal floor via CPI in the same transaction.
...
<Note>
  This account exists to make the RWT-into-Nexus path **atomic** with principal-floor accounting. Without it, RWT would have to first land in a regular wallet's ATA, then a separate transaction would call DEX `nexus_deposit`. With `LiquidityHolding`, the entire flow is a single instruction (`withdraw_liquidity_holding`) that bundles the SPL transfer with the principal-floor update via CPI.
</Note>
...
| LiquidityHolding | `"liquidity_holding"` | Singleton — RWT staging for the Nexus drain |
...
<Warning>
  **Do NOT use a naive current-balance snapshot.** A formula like `cumulative_amount = total_funded * holder_balance / total_eligible_supply` rewards anyone holding OT at publish time with a share of ALL historical deposits, including deposits that arrived before they acquired OT. This creates a front-running vector around announced distributions.
</Warning>
...
1. The bot listens for `DistributorFunded` / `StreamConverted` events on-chain.
2. At the slot of the fund event, the bot snapshots all OT holder balances via `getProgramAccounts`. This requires an archival RPC tier (e.g. Helius / Triton), since free-tier public RPC does not retain old program accounts.
3. The snapshot includes: regular wallets, **RWT Vault PDA** (holds OT as portfolio backing), **DEX Pool PDAs** (pool vaults hold OT with the pool PDA as authority), and **OtTreasury PDA** (may hold OT received from governance).
4. Filter applied per snapshot: only holders with ≥ $100 total protocol holdings **at that snapshot's slot**.
5. The non-eligible share (< $100) for that snapshot is allocated to the **ARL OtTreasury PDA** leaf — protocol revenue.
6. The snapshot is persisted: `{distributor, deposit_epoch, slot, balances: {holder → balance}, total_eligible}`.
...
2. Verify the invariant `Σ cumulative_amount[h] == total_funded`. Any integer-rounding remainder (bounded by `N_deposits`) is added to the ARL OtTreasury leaf.
3. Build the merkle tree: leaf = `sha256(address_bytes || cumulative_amount_le_bytes)` with canonical ordering (lower hash first).
4. Publish the root on-chain via `publish_root` with `max_total_claim = total_funded`.

<Note>
  The on-chain invariants `max_total_claim == total_funded` and `total_claimed ≤ max_total_claim` hold regardless of how `cumulative_amount` is computed off-chain — the contract verifies only the proof, not how the cumulative was derived. Switching algorithms (e.g. naive → per-deposit → TWAB) is an off-chain concern; no contract redeploy is needed.
</Note>
...
- **Independent verifiers** (strongly recommended). Two or more separate services — team-run and/or community-run — independently compute the root and post its hash to Discord, Telegram, or an on-chain log. A mismatch triggers an alert and rotation of `publish_authority` via `update_publish_authority`.
...
- [Liquidity Nexus](/architecture/liquidity-nexus) — subsystem-level overview of the protocol-owned LP and its principal-lock invariant
- [RWT Engine contract](/contracts/rwt-engine) — `claim_yield` 70 / 15 / 15 split; the 15% liquidity slice lands in the `LiquidityHolding` RWT ATA defined here
