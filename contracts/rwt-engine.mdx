...
<AccordionGroup>
  ...
  <Accordion title="update_distribution_config">
    ...
    | `liquidity_destination` | `Pubkey` | `LiquidityHolding` PDA RWT ATA (Yield Distribution program). Subsequently drained into the Liquidity Nexus via `withdraw_liquidity_holding` (single TX, atomic principal-floor update). |
  ...
  <Accordion title="claim_yield">
    ...
    **Logic:**
    ...
    5. Calculate splits (in RWT tokens):
       ...
       - `liquidity_share = claimed * liquidity_bps / 10,000` (→ `liquidity_dest` = `LiquidityHolding` PDA RWT ATA in Yield Distribution; subsequently drained into the Liquidity Nexus via `withdraw_liquidity_holding`, single TX, atomic principal-floor update)
...
| `areal_fee_destination` | `Pubkey` | Where 0.5% mint fee goes — Areal Finance USDC ATA (static, immutable). The same on-chain Pubkey is referenced under the account name `dao_fee_account` in `mint_rwt`; the handler validates `dao_fee_account.key == vault.areal_fee_destination`. |
...
| `liquidity_destination` | `Pubkey` | `LiquidityHolding` PDA RWT ATA (Yield Distribution program). Subsequently drained into the Liquidity Nexus via `withdraw_liquidity_holding` — single TX, atomic update of the Nexus principal floor. |
...
| Vault RWT | LiquidityHolding RWT ATA (15%) → Nexus via `withdraw_liquidity_holding` (single TX, atomic) | claim_yield → withdraw_liquidity_holding | Crank → Authority |
...
- [Liquidity Nexus](/architecture/liquidity-nexus) — subsystem-level overview of the protocol-owned LP, the principal-lock invariant, and the deposit lanes that the 15% liquidity slice feeds into
- [Yield Distribution contract](/contracts/yield-distribution) — `LiquidityHolding` PDA and the atomic-drain instruction
