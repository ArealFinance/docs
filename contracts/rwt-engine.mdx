...
<AccordionGroup>
  ...
  <Accordion title="update_distribution_config">
    ...
    | `liquidity_destination` | `Pubkey` | LiquidityHolding PDA RWT ATA (YD program). Layer 9 atomic drain via `withdraw_liquidity_holding` |
  ...
  <Accordion title="claim_yield">
    ...
    **Logic:**
    ...
    5. Calculate splits (in RWT tokens):
       ...
       - `liquidity_share = claimed * liquidity_bps / 10,000` (→ liquidity_dest = YD `LiquidityHolding` PDA RWT ATA. Layer 9 atomic drain: `withdraw_liquidity_holding` → DEX `nexus_record_deposit`)
...
| `areal_fee_destination` | `Pubkey` | Where 0.5% mint fee goes (static, immutable). **Dual-role alias:** `convert_to_rwt` callers see this same on-chain account under the name `dao_fee_account` — both names refer to the same Pubkey on `RwtVault`. The on-chain handler validates `dao_fee_account.address == vault.areal_fee_destination` (`mint_rwt.rs:65`). Surfaced as `getRwtDaoFeeAccount(vault)` in the dashboard reader for naming clarity. |
...
| `liquidity_destination` | `Pubkey` | LiquidityHolding PDA RWT ATA (YD program). Layer 9 atomic drain via `withdraw_liquidity_holding` updates Nexus principal floor |
...
| Vault RWT | LiquidityHolding RWT ATA (15%) → Nexus via `withdraw_liquidity_holding` (atomic, single-TX) | claim_yield → withdraw_liquidity_holding | Crank → Authority |
...
[Layer 10 Bootstrap](/architecture/layer10-bootstrap) covers the cross-contract authority chain (RWT-to-Multisig propose-and-accept in Phase 7) and the end-to-end E2E scenarios — including the Happy Path (`mint_rwt` + `admin_mint_rwt` + `claim_yield` 70/15/15 split) and the Emergency scenario's `adjust_capital` writedown with NAV-decrease assertion.
